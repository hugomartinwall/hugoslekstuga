...
/**
 * Fixed-timestep loop (Overrun's, verbatim shape). The sim advances in exact
 * TICK_MS steps regardless of display refresh rate; rendering happens once
 * per animation frame with an interpolation alpha in [0, 1).
 */
...
export interface Loop {
  ...
  /**
   * Presentation-side time dilation (boss-kill slow-mo): scales how fast
...
export function createLoop(cb: LoopCallbacks): Loop {
  ...
  return {
    ...
    stop() {
      // Unmount path (the game lives inside a React page) — without this
